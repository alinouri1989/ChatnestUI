...
import { downloadImageFromBase64 } from "../../../helpers/downloadImageFromBase64";
...
import { useGenerateImageMutation, useGenerateTextMutation } from "../../../store/Slices/ChatNestAi/ChatNestAiApi";
import { opacityAndTransformEffect } from "../../animations/animations";
...
export const AIModal = ({ chatId, isOpen, onClose, buttonRef }) => {
    ...
    const sendMessage = async (contentType, content) => {
        try {
            const chatType = getChatType(location.pathname);
            await createData(`Chat/${chatId}/Messages?chatType=${encodeURIComponent(chatType)}`, {
                method: "POST",
                body: { ContentType: contentType, Content: content, ThumbnailUrl: "" },
            });
            onClose();
        } catch {
