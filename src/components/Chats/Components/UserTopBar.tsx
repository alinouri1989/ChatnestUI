...
function UserTopBar({ isSidebarOpen, toggleSidebar, chatId, recipientProfile, recipientId }) {
  const dispatch = useDispatch();
  const location = useLocation();
  const { callConnection, typingUsersByChat } = useSignalR();
  ...
  const isRecipientTyping =
    !!recipientId && (typingUsersByChat?.[chatId] ?? []).includes(recipientId);
  const status = isUserOnline(
    recipientProfile.lastConnectionDate,
  ...
  const subtitle =
    isSavedMessagesChat
      ...
      : status === "online"
        ? "آنلاین"
        : formatDateForLastConnectionDate(
          lastConnectionDate,
          recipientProfile.isOnline
        );
      ...
      <div className="user-info">
        ...
        <div onClick={toggleSidebar} className="image-box">
          {isSavedMessagesChat ? (
            ...
            <>
              <img
                src={(recipientProfile.profilePhoto != "" && recipientProfile.profilePhoto) ? recipientProfile.profilePhoto : defaultProfilePhoto}
