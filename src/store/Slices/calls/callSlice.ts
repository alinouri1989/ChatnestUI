...
const handleCall = (data, dispatch, userId, isIncoming) => {
    const { callId, callType, profile, ...callerData } = data;
    const ignoredKeys = new Set(["callerId", "recipientId", "profileUserId"]);
    const callerProfileKey = Object.keys(callerData).find(
        key => key !== userId && !ignoredKeys.has(key)
    );
    const callerProfile = profile ?? (callerProfileKey ? callerData[callerProfileKey] : null);
